...
//czas timsort
double measure_timsort_time(vector<int> arr) {
...
int main() {
    std::ofstream file("results\\res_timsort.txt");
    if (!file.is_open()) {
        cerr << "Error!\n";
        return 1;
    }
    ...
    for(int size : sizes) {
        ...
        for(int i = 0; i < 100; ++i) {
            ...
            timsort_time += measure_timsort_time(arr);
        }
        //uśrednienie czasu
        timsort_time /= 100;
        file << "Losowy\t" << size << "\t" << timsort_time << endl; //zapisuje wyniki do pliku
    }
    ...
    for (int size : sizes) {
        ...
        for (double percentage : percentages) {
            ...
            timsort_time /= 100;
            file << "Posortowana " <<  (percentage * 100) << " %\t" << size << "\t" << timsort_time << endl;
        }
    }
    ...
    for(int size : sizes) {
        ...
        timsort_time /= 100;
        file << "Posortowana odwrotnie\t" << size << "\t" << timsort_time << endl;
    }
